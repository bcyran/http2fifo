...
use http2fifo::{config::Config, error::Error, mount, mount_all};
...
fn spawn_reader(path: PathBuf) -> JoinHandle<Vec<u8>> {
    tokio::spawn(async move {
        // Wait for mount to create the FIFO.
        while !path.exists() {
...
/// Mount B: large body that cannot finish before cancellation → `Err(Cancelled)`.
#[tokio::test]
async fn fail_fast() {
    let server_a = MockServer::start().await;
    Mock::given(method("GET"))
